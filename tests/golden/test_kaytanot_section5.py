...
'בחוק הקייטנות (רישוי ופיקוח), התש"ן–1990, בסעיף 5,
במקום "מאסר ששה חדשים" יבוא "מאסר שנה".' עם side_heading="תיקון סעיף 5",
number="1." (הוראת התיקון הראשונה ממוספרת כרגיל, ראו תיקון 2026-09).
en-dash לפני 1990 (לא מקף רגיל) - ראו docs/drafting-rules.md §5.7.
מראה המקום (הערת השוליים) מפוצל ל-text/text_after כדי לשבת מיד אחרי
השנה, לפני "(להלן..." - ראו engine._render_mutation.

**שינוי מכוון (2026-09-17): אין כאן "(להלן – החוק העיקרי)".**
עד לתאריך הזה הטסט ציפה לקיצור, בעקבות תיקון משימה 6א שקבע שהוא
נדרש בכל פעם ראשונה שהחוק מוזכר. **ביקורת מול 40 הצעות חוק אמיתיות
שהונחו בכנסת ה-25 הראתה שזה שגוי:** מתוך 22 ההצעות המתקנות, רק 10
מגדירות את הקיצור, ו-12 פותחות ישירות בשם החוק המלא. התנאי האמיתי
הוא **יותר מהוראת תיקון אחת** לאותו חוק - הקיצור נועד לחסוך חזרה,
ובתיקון יחיד אין על מה לחסוך.

ההצעה הזו מתקנת סעיף אחד, ולכן הניסוח הנכון פותח ישירות בשם החוק.
זה תיקון ולא רגרסיה. ראו drafting-rules.md §5.8 ו-
engine._drop_principal_law_alias_if_single.
"""
...
EXPECTED_LINES = [
    Line(
        ...
        text_after=', בסעיף 5, במקום "מאסר ששה חדשים" יבוא "מאסר שנה".',
        footnotes=["kaytanot"],
        depth=0,
