...
def main() -> int:
    ...
    # **שבר מוצג ולא מוסר** (ברק, 22.9.2026). הסרת המעטפת הייתה
    # הופכת "3/4" ל-"3 4" - שינוי משמעות, לא שינוי עיצוב.
    checks.append(("שבר: מונה / מכנה",
                   render("{{שבר|מונה|מכנה}}") == "מונה / מכנה"))
    checks.append(("שבר: ארגומנט עיצוב בשם יורד, לא נחשב כמונה",
                   render("{{שבר|כיוון=rtl|מונה|מכנה}}") == "מונה / מכנה"))
    checks.append(("sfrac מתנהג כמו שבר",
                   render("{{sfrac|גודל=100%|א|ב}}") == "א / ב"))
    checks.append(("שבר עם תבניות מקוננות במכנה",
                   render("{{שבר|א|{{בלוק|ב}} {{מוגדל|×}} {{בלוק|ג}}}}") == "א / ב × ג"))
