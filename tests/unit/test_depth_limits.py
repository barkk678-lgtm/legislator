...
CASES = [
    ...
    # עומק 3 - פסקה בתוך הגדרה. המכולה כאן **במילים** ולא בסוגריים
    # (§7.9.1), כי להגדרה אין מספר. נפתח במשימה 62.
    ("maavak-birgunei-plisha", "maavak-2003", "maavak-2003/פרק א/s1/p1/1",
     "paragraph", "(1א)", 'בסעיף 1, בהגדרה "ארגון פשיעה", אחרי פסקה (1) יבוא:'),
    # עומק 4 - הגדרה בתוך סעיף קטן ממוספר: הכתובת מרכיבה מכולה
    # בסוגריים **וגם** פסוקית במילים.
    ("penal", "penal", "penal/חלק ב/פרק ט/פרק ט סימן ה/s295/ב1/p2/1",
     "paragraph", "(1א)",
...
#
# **עודכן במשימה 62:** פסקה בתוך הגדרה עברה מכאן ל-CASES. היא
# הייתה חסומה מטעם נכון (המכולה בסוגריים מדלגת על ההגדרה), ומה
# שפתח אותה הוא ניסוח ההגדרה במילים - `בהגדרה "X"` לפי §7.9.1 -
# ולא ביטול החסימה.
#
# מה שנשאר חסום: שרשרת שיש בה צומת **בלי מספר ושאינו הגדרה**.
# בחוק העונשין ובמאבק בארגוני פשיעה יש פסקאות ממוספרות מתחת
# לפסקה בלי תווית; שם הכתובת באמת הייתה מדלגת על רמה שלמה.
BLOCKED = [
    ("maavak-birgunei-plisha", "maavak-2003", "maavak-2003/פרק ג/s5/p0/1",
     "paragraph", "בלי מספר"),
]
