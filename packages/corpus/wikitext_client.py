...
def is_primary_legislation_title(title: str) -> bool:
    """True רק לחוקים וחוקי-יסוד - **לא** תקנות/צווים/כללים/פקודות/
    הוראות/אכרזות וכו'. הוכרע במפורש (2026-09-13, ברק) אחרי שנמצא
    בפועל: "קטגוריה:בוט חוקים" מכילה גם חקיקת משנה (תקנות/צווים -
    שמחוקקת הממשלה/שר מכוח הסמכה בחוק, לא הכנסת) - 73% מהקטגוריה,
    לא רק חוקים. חקיקת משנה **לא** ברת-תיקון בהצעת חוק (מטרת
    הפרויקט) - רק חוקים וחוקי-יסוד, שמחוקקים על ידי הכנסת, הם.
    פקודות (חקיקה ראשית מתקופת המנדט, עדיין בתוקף) הוצאו במפורש
    למרות שיש להן מעמד דומה לחוק - "רק חוקים וחוקי יסוד. לא שום
    דבר אחר" (ברק, מילולית). ראו docs/strategy/decisions.md.

    **היקף זה זמני, לא קבוע**: ייתכן שיום אחד נרצה גם תקנות/צווים
    (ברק, 2026-09-14) - אבל לא בשלב הזה."""
    return title.startswith("חוק ") or title.startswith("חוק-יסוד:") or title == "חוק"
