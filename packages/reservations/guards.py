...
_CLASSIFIER_INSTRUCTIONS = f"""אתה בודק נוסח של הסתייגות להצעת חוק לפי סעיף 86(ד)(2) לתקנון הכנסת:
...
def screen_content(
    ...
    # **fail-closed, והתאמה מדויקת ולא תחילית.** `startswith("לא")`
    # היה חור fail-open: "לאחר בדיקה…" מתחיל ב"לא" ועובר. בעברית
    # "לא" הוא תחילית של מילים רבות, וזה בדיוק דפוס הבאג שכבר
    # תועד ב-CLAUDE.md (כל regex לזיהוי ערך חייב טסט שלילי).
    # רק התשובה "לא" עצמה, עם פיסוק אופציונלי, עוברת.
    if answer.rstrip(".,!׃:״\"' ") == "לא":
        return ALLOWED
