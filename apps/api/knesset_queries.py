...
def term_in(term: str, title: str) -> bool:
    ...
    - **מונח של כמה מילים:** "בתי ספר" לא נמצא ב"בבתי **ה**ספר" - ושורה
      ...
      מילה נבדקת לחוד.
    - **מונח קצר:** "מור" (מורים) הוא תת-מחרוזת של "ח**מור**" - ו"מחסור
      **חמור** במוסכניקים" נכנס לנושא על מורים. אחר כך, כשדרשתי רק
      שיפתח מילה: "תקנ" (תקנים) תפס את "ה**תקנ**ת מצלמות". בשלוש אותיות
      העברית דו-משמעית מדי, ולכן **מונח של עד 3 אותיות חייב להיות מילה
      שלמה** (אחרי עד שלוש אותיות שימוש: ו/ה/ב/ל/מ/ש/כ): "בתקן" כן,
      "התקנת" ו"חמור" לא. ההנחיה מבקשת בשביל זה צורות מלאות, לא גזעים."""
    t = _norm_title(title)
    for word in _norm_title(term).split():
        if len(word) > _SHORT_STEM:
            if word not in t:
                return False
        elif not re.search(rf"(?:^|[^א-ת])[{_PREFIXES}]{{0,3}}{re.escape(word)}(?![א-ת])", t):
            return False
    return True
